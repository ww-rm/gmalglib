#include <stdint.h>
#include <gmalglib/sm4.h>

static const
uint8_t S_BOX[256] = {
    
};

static const
uint32_t CK[32] = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};

static inline
uint32_t ROL(uint32_t X, uint32_t count)
{
    return (X << count) | (X >> (32 - count));
}

static inline
uint32_t FROM_BE(const uint8_t* bytes)
{
    return ((uint32_t)bytes[0] << 24) | 
           ((uint32_t)bytes[1] << 16) | 
           ((uint32_t)bytes[2] << 8)  |
           ((uint32_t)bytes[3]);
}

static inline
void TO_BE(uint32_t word, uint8_t* bytes)
{
    bytes[0] = (uint8_t)(word >> 24);
    bytes[1] = (uint8_t)(word >> 16);
    bytes[2] = (uint8_t)(word >> 8);
    bytes[3] = (uint8_t)(word);
}

static inline
uint32_t BS(uint32_t X)
{
    return ((uint32_t)S_BOX[(X >> 24) & 0xFF] << 24) |
           ((uint32_t)S_BOX[(X >> 16) & 0xFF] << 16) |
           ((uint32_t)S_BOX[(X >>  8) & 0xFF] <<  8) |
           ((uint32_t)S_BOX[(X      ) & 0xFF]);
}

static inline
uint32_t T0(uint32_t X)
{
    X = BS(X);
    return X ^ ROL(X, 2) ^ ROL(X, 10) ^ ROL(X, 18) ^ ROL(X, 24);
}

static inline
uint32_t T1(uint32_t X)
{
    X = BS(X);
    return X ^ ROL(X, 13) ^ ROL(X, 23);
}

static inline
void ExpandKey(const uint8_t* key, uint32_t* round_key)
{
    uint32_t i;
    uint32_t K0 = FROM_BE(key) ^ 0xa3b1bac6;
    uint32_t K1 = FROM_BE(key + 4) ^ 0x56aa3350;
    uint32_t K2 = FROM_BE(key + 8) ^ 0x677d9197;
    uint32_t K3 = FROM_BE(key + 12) ^ 0xb27022dc;

    for (i = 0; i < 32; i += 4)
    {
        round_key[i    ] = K0 = K0 ^ T1(K1 ^ K2 ^ K3 ^ CK[i    ]);
        round_key[i + 1] = K1 = K1 ^ T1(K2 ^ K3 ^ K0 ^ CK[i + 1]);
        round_key[i + 2] = K2 = K2 ^ T1(K3 ^ K0 ^ K1 ^ CK[i + 2]);
        round_key[i + 3] = K3 = K3 ^ T1(K0 ^ K1 ^ K2 ^ CK[i + 3]);
    }
}

void SM4_Init(SM4* self, const uint8_t* key)
{
    ExpandKey(key, self->round_key);
}

void SM4_Encrypt(SM4* self, const uint8_t* block_in, uint8_t* block_out)
{
    uint32_t i;
    uint32_t X0 = FROM_BE(block_in);
    uint32_t X1 = FROM_BE(block_in + 4);
    uint32_t X2 = FROM_BE(block_in + 8);
    uint32_t X3 = FROM_BE(block_in + 12);

    for (i = 0; i < 32; i += 4)
    {
        X0 = X0 ^ T0(X1 ^ X2 ^ X3 ^ self->round_key[i    ]);
        X1 = X1 ^ T0(X2 ^ X3 ^ X0 ^ self->round_key[i + 1]);
        X2 = X2 ^ T0(X3 ^ X0 ^ X1 ^ self->round_key[i + 2]);
        X3 = X3 ^ T0(X0 ^ X1 ^ X2 ^ self->round_key[i + 3]);
    }

    TO_BE(X3, block_out);
    TO_BE(X2, block_out + 4);
    TO_BE(X1, block_out + 8);
    TO_BE(X0, block_out + 12);
}

void SM4_Decrypt(SM4* self, const uint8_t* block_in, uint8_t* block_out)
{
    int32_t i;
    uint32_t X0 = FROM_BE(block_in);
    uint32_t X1 = FROM_BE(block_in + 4);
    uint32_t X2 = FROM_BE(block_in + 8);
    uint32_t X3 = FROM_BE(block_in + 12);

    for (i = 31; i >= 0; i -= 4)
    {
        X0 = X0 ^ T0(X1 ^ X2 ^ X3 ^ self->round_key[i    ]);
        X1 = X1 ^ T0(X2 ^ X3 ^ X0 ^ self->round_key[i - 1]);
        X2 = X2 ^ T0(X3 ^ X0 ^ X1 ^ self->round_key[i - 2]);
        X3 = X3 ^ T0(X0 ^ X1 ^ X2 ^ self->round_key[i - 3]);
    }

    TO_BE(X3, block_out);
    TO_BE(X2, block_out + 4);
    TO_BE(X1, block_out + 8);
    TO_BE(X0, block_out + 12);
}
